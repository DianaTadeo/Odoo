...
class Certificate(models.Model):
    ...
    cer = fields.Binary(string='Certificado', filters='*.cer,*.certificate,*.cert', required=True, attachment=False) #Content
    key = fields.Binary(string='Llave privada', filters='*.key', required=True) #key
    ...
    def get_valid_certificate(self):
        ...
        for record in self:
            date_start = str_to_datetime(record.start_date)
            date_end = str_to_datetime(record.end_date)
