...
class AccountPayment(models.Model):
    ...
    cfd_mx_payment_method_id = fields.Many2one('cfd_mx.metodopago', string=u'Metodo de Pago')
    ...
    def _create_cfdi_payment(self):
        ...
        partner = self.journal_id._address_issued_id or self.company_id.partner_id.commercial_partner_id
        tz = self.env['account.invoice']._get_timezone(
            partner.state_id.code)
        date_mx = datetime.now(tz)
        ...
        fil.write(cfdi)
        fil.close()
        raise UserError('Hasta aquí')
        # -Compute cadena
        tree = self.get_xml_etree(cfdi)
        cadena = self.env['account.invoice'].generate_cadena(
    ...
    def _create_cfdi_values(self):
        ...
        values = {
            ...
            'fiscal_regime': self.company_id.cfd_mx_fiscal_regime,
            'invoice': invoice_obj,
        }
    ...
    def payment_data(self):
        ...
        raise UserError(self.cfd_mx_payment_method_id)
...
class AccountPaymentRegister(models.TransientModel):
    ...
    cfd_mx_payment_method_id = fields.Many2one(
        'cfd_mx.payment.method',
        string='Payment Way',
    ...
    def _prepare_payment_vals(self, invoice):
        ...
        res.update({
            'cfd_mx_payment_method_id': self.cfd_mx_payment_method_id.id,
            'cfd_mx_partner_bank_id': self.cfd_mx_partner_bank_id.id,
        })
        return res
"""