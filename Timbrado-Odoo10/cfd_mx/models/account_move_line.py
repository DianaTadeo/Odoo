...
class AccountMoveLine(models.Model):
    ...
    uuid = fields.Char(string='Timbre fiscal', related='payment_id.cfd_mx_cfdi_uuid')
    ...
    def get_xml(self):
        ...
        if self.payment_id and self.payment_id.cfdi_timbre_id:
            xml_id = self.env["ir.attachment"].search([('res_model', '=', 'account.payment'), ('res_id', '=', self.payment_id.id), ('type', '=', 'binary'), ('name', 'like', self.payment_id.cfd_mx_cfdi_name )])
    ...
    def get_pdf(self):
        self.ensure_one()
        if self.payment_id and self.payment_id.cfdi_timbre_id:
            return self.env['report'].get_action(self.payment_id, 'cfd_mx.report_cfdi_payment_receipt_document')
        else:
            raise UserError("No es una Factura CFDI de Pago")
    ...
    def get_email(self):
        ...
        self.ensure_one()
        if self.payment_id and self.payment_id.cfdi_timbre_id:
            template = self.env.ref('cfd_mx.email_template_payment_receipt', False)
            #res= self.get_pdf()
            #logging.info(res)
            ...
            ctx = dict(
                ...
                custom_layout="cfd_mx.email_notification_paynow"
            )
            return {
